...
def test_individual_rules(validator):
    ...
    is_valid, message = validator.rules_lookup["jupyter_commands"].validation_fn("!pip install numpy")
    assert is_valid

    is_valid, message = validator.rules_lookup["jupyter_commands"].validation_fn("!rm -rf /")
    assert not is_valid

    # Test dangerous builtins rule
    is_valid, message = validator.rules_lookup["dangerous_builtins"].validation_fn("print('hello')")
    assert is_valid

    is_valid, message = validator.rules_lookup["dangerous_builtins"].validation_fn("eval('2+2')")
    assert not is_valid
...
def test_package_version_validation(validator):
    test_cases = [
        ...
        ("!pip install pillow>9.0.0,<10.0.0", True, "Valid version range should pass"),
        # Multiple packages
        ("!pip install pillow>=9.0.0 numpy>=1.22.2", True, "Multiple valid packages should pass"),
        ("!pip install pillow>=9.0.0 numpy==1.20.0", False, "One invalid version should fail"),
        # Different pip install syntaxes
        ("!python -m pip install pillow>=9.0.0", True, "python -m pip syntax should work"),
        # Blocked packages
        ("!pip install crypto", False, "Blocked package should fail"),
        ("!pip install crypto==1.0.0", False, "Blocked package with version should fail"),
        ("!pip install pillow>=9.0.0 crypto", False, "Valid package with blocked package should fail"),
        # Package without version
        ("!pip install requests", True, "Package without version specification should pass"),
        ("!pip install pillow", True, "Package without version should pass when has allowed versions"),
        # Conda syntax
        ("!conda install pillow>=9.0.0", True, "Conda syntax should work"),
        ("!conda install pillow==8.0.0", False, "Conda syntax should enforce versions"),
        # Edge cases
        ("!pip install pillow>=9.0.0 # some comment", True, "Comments should be handled"),
        ("!pip install --upgrade pillow>=9.0.0", True, "Pip flags should be handled"),
        ("!pip install -U pillow>=9.0.0", True, "Short pip flags should be handled"),
        ("!pip install", True, "Empty pip install should pass"),
        ("print('!pip install pillow')", True, "Pip command in string should pass"),
        # Invalid syntax
        ("!pip install pillow>=invalid", False, "Invalid version syntax should fail"),
    ]
...
print('Hello')
        """,
            True,
            "Valid package with safe code should pass",
        ),
        (
            """
!pip install pillow>=9.0.0
import sys  # forbidden import
        """,
            False,
            "Valid package with forbidden import should fail",
        ),
        (
            """
...
import numpy as np
        """,
            False,
            "Blocked package with safe import should fail",
        ),
    ]
...
def test_package_name_variants(validator):
    ...
    test_cases = [
        ("!pip install scikit-learn>=1.0.0", True, "Package with hyphen should work"),
        ("!pip install python_dateutil>=2.0.0", True, "Package with underscore should work"),
        ("!pip install pillow[extra]>=9.0.0", True, "Package with extras should work"),
        ("!pip install Flask", False, "Blocked package with capital letters should fail"),
        ("!pip install PILLOW>=9.0.0", True, "Package name should be case insensitive"),
    ]
